...
export class VehicleBalanceCalculator {
  constructor() {
    ...
    this.generators = [
      { code: 'GER0003', name: 'ETA PIRAPAMA' },
      { code: 'GER0004', name: 'EEAB PIRAPAMA' },
      { code: 'GER0005', name: 'ETA GURJAÚ' }
    ];
    ...
    this.container.innerHTML = `
      <div class="bg-white rounded-xl shadow-2xl w-[92vw] sm:w-96 flex flex-col max-h-[85vh] sm:max-h-[80vh] border border-gray-200 animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50 custom-scrollbar">
          ...
          <!-- Veículos -->
          <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Veículos</p>
      ...
      return `
          ...
          <div class="border-t border-dashed border-gray-200 pt-3 mt-1">
            <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">Geradores</p>
            ${this.generators.map(gen => {
      const plate = gen.code;
      const value = this.values[plate] || '';
      const valueStr = value ? value.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) : '';
      return `
              <div class="flex items-center gap-2 bg-white p-2 rounded-lg border border-gray-100 shadow-sm focus-within:ring-2 focus-within:ring-green-500 transition-shadow mb-2">
                <div class="flex flex-col w-20 shrink-0">
                  <span class="font-mono font-medium text-gray-700 text-xs bg-gray-100 px-2 py-0.5 rounded text-center select-all leading-tight">${plate}</span>
                  <span class="text-[9px] text-gray-400 text-center leading-tight mt-0.5 truncate">${gen.name}</span>
                </div>
                <div class="flex-1 relative">
                  <span class="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400 text-sm">R$</span>
                  <input 
  ...
  async handleCopy() {
    ...
    const generatorLines = this.generators
      .filter(gen => this.values[gen.code] && this.values[gen.code] > 0)
      .map(gen => `${gen.code} (${gen.name}) = ${this.formatCurrency(this.values[gen.code])}`);

    if (generatorLines.length > 0) {
      text += `\nGeradores:\n`;
      generatorLines.forEach(line => { text += `${line}\n`; });
    }
