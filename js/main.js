...
import { showNotification, showError, toggleModal, htmlToPlainText, copyToClipboard } from './utils.js';
...
import { renderMaintenanceModal, renderMiscModal, renderRequestMaintenanceModal, renderEmailReportModal } from './ui/modals.js';
...
async function initializeApp() {
  ...
  document.getElementById('copyEmailBtn').addEventListener('click', async () => {
    const content = document.getElementById('emailReportContent');
    if (await copyToClipboard(htmlToPlainText(content.innerHTML))) {
      showNotification('E-mail copiado com sucesso!');
      setTimeout(() => toggleModal('emailReportModal', false), 1000);
    }
  });
