...
function onFileProcessed(table) {
  ...
  renderSummary(table, maintenanceState, materialsState);
...
async function initializeApp() {
  ...
  const allPlatesAndGenerators = [...desiredPlates, ...generators.map(g => g.code)];
  allPlatesAndGenerators.forEach(plate => {
  ...
  if (fabToggleBtn) {
    fabToggleBtn.addEventListener('click', (e) => {
      ...
      fabToggleBtn.classList.toggle('active');
    });

    // Close menu when clicking outside
    document.addEventListener('click', (e) => {
      if (buttonContainer.classList.contains('expanded') && !buttonContainer.contains(e.target)) {
        buttonContainer.classList.remove('expanded');
        fabToggleBtn.classList.remove('active');
      }
    });

    // Close menu when clicking an action button (UX choice)
    const actionButtons = buttonContainer.querySelectorAll('.custom-btn:not(.fab-toggle)');
